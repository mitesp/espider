...
"""
A Timeslot is a date x duration block during which a program is happening. The
duration should be the smallest denomination of time that schedule offsets need
...
TODO(constraint): start.time and end.time could only be a fixed set of values
(potentially on the hour and half hour, maybe quarter hour)
...
(6 timeslots/day x 7 days/program)
    2020/02/29 13:00–13:30, 13:30–14:00, ... , 15:30–16:00 (for a total of 6)
...
"""
A ScheduledEvent represents a section of a class scheduled at a timeslot of a
program TODO: in a particular classroom.

A 2-hour section scheduled in a program with 1-hour timeslot durations would be
represented by two ScheduledEvent objects. A 1.5-hour section scheduled in a
program with half-hour timeslot durations would be represented by three
